# scenes/bad_ending.py
import pygame
import os
from settings import *
from engine.dialog_box import DialogBox
from narrative.scripts import BAD_ENDING_DIALOGUE

class BadEndingScene:
    def __init__(self, screen):
        self.screen = screen
        self.dialog_box = DialogBox()
        self.background = pygame.image.load(os.path.join(IMAGES_DIR, "final_malo.png")).convert()
        ...
        # Música de fondo
        try:
            pygame.mixer.music.load(os.path.join(AUDIO_DIR, "finalmalo.mp3"))
            pygame.mixer.music.play(-1)
        except:
            pass

    def load_next_dialog(self):
        if self.dialog_index >= len(BAD_ENDING_DIALOGUE):
            self.running = False
            return

        dialog = BAD_ENDING_DIALOGUE[self.dialog_index]
